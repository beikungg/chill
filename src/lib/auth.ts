...
export const auth = betterAuth({
  ...
  onAPIError: {
    ...
    onError: (error, ctx) => {
      ...
      // 类型安全：将 error 视为 any 以访问属性
      const err = error as any;
      // 特别处理状态不匹配错误
      if (
        err.message?.includes('State Mismatch') ||
        err.message?.includes('Verification not found')
      ) {
        console.error(
          'OAuth State Mismatch detected - this may be due to mobile browser context changes or repeated authentication attempts'
        );
      }
      // 处理网络连接错误
      if (
        ...
        console.error(
          'Network connection error to OAuth provider - check firewall/proxy settings'
        );
        console.error('Host:', err.cause?.host, 'Port:', err.cause?.port);
      }
    },
  },
});
