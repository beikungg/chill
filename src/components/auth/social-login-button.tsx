  ...
  const onClick = async (provider: 'google' | 'github') => {
    try {
      ...
      const authCookies = document.cookie
        ...
        .filter(
          (cookie) =>
            cookie.trim().startsWith('better-auth') ||
      ...
      await authClient.signIn.social(
        {
           ...
           */
          errorCallbackURL: `${Routes.AuthError}?error=social_login_failed&provider=${provider}`,
          /**
        ...
        },
        {
          ...
          onError: (ctx) => {
            ...
            // 特别处理状态不匹配错误
            if (
              ctx.error.message?.includes('State Mismatch') ||
              ctx.error.message?.includes('please_restart_the_process')
            ) {
              // 清除所有认证相关 cookie
              ...
              // 重定向到带有特定错误参数的错误页面
              window.location.href = `${Routes.AuthError}?error=please_restart_the_process&provider=${provider}`;
            }
          },
        }
      );
