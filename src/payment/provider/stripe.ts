...
export class StripeProvider implements PaymentProvider {
  ...
  constructor() {
    ...
    if (!apiKey) {
      console.warn(
        '⚠️ STRIPE_SECRET_KEY environment variable is not set. Payment features will be disabled.'
      );
      // Create a dummy Stripe instance to avoid breaking the app
    ...
    if (!webhookSecret) {
      console.warn(
        '⚠️ STRIPE_WEBHOOK_SECRET environment variable is not set. Webhook features will be disabled.'
      );
      this.webhookSecret = '';
    } else {
  ...
  public async getSubscriptions(
    ...
    if (!this.stripe) {
      console.warn('Stripe is not configured, skipping getSubscriptions');
      return [];
    }
  ...
  public async handleWebhookEvent(
    ...
    if (!this.stripe || !this.webhookSecret) {
      console.warn('Stripe is not configured, skipping webhook event');
      return;
    }
